...
public class RpcClient {
  ...
  public void setServiceInterface(Class clazz, NamingOptions namingOptions) {
    if (this.serviceInterface != null) {
      throw new RpcException(
          "serviceInterface must not be set repeatedly, please use another RpcClient");
    }
  ...
  /**
   * select instance by load balance and select channel from the instance. when user call this
   * function explicitly, he should return the channel to avoid connection leak.
   *
   * @return netty channel
   */
  public Channel selectChannel() {
    boolean isHealthInstance = true;
    BrpcChannel brpcChannel = loadBalanceStrategy
        .selectInstance(endPointProcessor.getHealthyInstances());
    if (brpcChannel == null) {
    ...
    try {
      ...
      String errMsg = String
          .format("channel pool is exhausted, and double maxTotalConnection,server=%s:%d",
  ...
  public Channel selectChannel(Endpoint endpoint) {
    ...
    try {
      ...
      throw new RpcException(RpcException.NETWORK_EXCEPTION,
          "select channel failed from " + endpoint);
    }
    if (!channel.isActive()) {
      brpcChannel.incFailedNum();
      // 如果连接不是有效的，从连接池中剔除。
  ...
  private void init(final RpcClientOptions options, List<Interceptor> interceptors,
    ...
    this.protocol = ProtocolManager.instance().init(options.getEncoding())
        .getProtocol(options.getProtocolType());
    ...
    this.workThreadPool = BrpcWorkThreadPoolInstance
        .getOrCreateInstance(rpcClientOptions.getWorkThreadNum());
    ...
    bootstrap
        .option(ChannelOption.CONNECT_TIMEOUT_MILLIS, rpcClientOptions.getConnectTimeoutMillis());
