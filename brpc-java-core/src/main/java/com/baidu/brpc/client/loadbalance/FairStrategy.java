...
/**
 * Fair load balance strategy aims to more reasonable distribution of traffic. The principle of
 * traffic distribution is not the same as pv, but the distribution of traffic according to
 * performance, the performance of the machine with multi-point traffic, the poor performance of the
 * machine with less point traffic, the same machine traffic with the same performance, and finally
 * the service performance of all nodes tends to balance, there will be no slow nodes to improve the
 * overall performance of the service.
 *
 * <p>Algorithm overview:
 * The fair load balancing policy dynamically adjusts the weight of each server and divides it by
 * weight. Rpc client maintains a latency window record rpc time for each server, after processing a
 * traffic, the server updates its own window. If the mean value of the service in the window > the
 * overall mean of the service, reduce its own weight. On the contrary, increase its own weight.
...
public class FairStrategy implements LoadBalanceStrategy {
  ...
  /**
   * The binary tree used to save weight number for each node. We use a CopyOnWriteArrayList to
   * safely lockless update the weight tree, The first element fo the CopyOnWriteArrayList is
   * useful. The leaf elements of the weight tree are the real rpc servers, and the parent elements
   * of the weight used to save the sum weight of its left and right child's weight. The root
   * element's weight sum is the total sum of every leaf element's weight.
   *
   * <p>The weight tree can achieve time complexity at the O(logN) level, and 1000 servers require
   * only 11 memory accesses.
   */
    ...
    if (timer == null) {
      synchronized (this) {
        if (timer == null) {
          ...
          latencyWindowSize = rpcClient.getRpcClientOptions()
              .getLatencyWindowSizeOfFairLoadBalance();
          activeInstancesRatio = rpcClient.getRpcClientOptions()
              .getActiveInstancesRatioOfFairLoadBalance();
  ...
  /**
   * Since the weight tree will update by a period of time, so if there's any invalid instance, the
   * business should notify the fair strategy.
   */
  ...
  /**
   * Update weight of each node of the tree. By create a new tree and insert into the head of the
   * {@link #treeContainer}
   */
  protected void updateWeightTree() {
    ...
    if (fullWindowInstances.size() < minInstancesNum
        || fullWindowInstances.size() * 1.0 / rpcClient.getHealthyInstances().size()
  ...
  /**
   * generate the tree by leaf nodes the leaf nodes are the real rpc servers the parent nodes used
   * to calculate the sum of it's children's weight
   *
