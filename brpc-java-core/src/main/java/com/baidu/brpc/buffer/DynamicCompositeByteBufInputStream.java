 ...
 * <p>
 * A read operation against this stream will occur at the {@code readerIndex} of its underlying
 * buffer and the {@code readerIndex} will increase during the read operation.  Please note that it
 * only reads up to the number of readable bytes of its underlying buffer.
 * <p>
...
public class DynamicCompositeByteBufInputStream extends InputStream implements DataInput {
   ...
   * @param releaseOnClose {@code true} means that when {@link #close()} is called then {@link
   * DynamicCompositeByteBuf#release()} will be called on {@code buffer}.
   */
