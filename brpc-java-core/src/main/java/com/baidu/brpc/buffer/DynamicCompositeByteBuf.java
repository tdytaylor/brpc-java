...
/**
 * dynamically composite multi {@link ByteBuf}, and it can be read just like a normal {@link
 * ByteBuf}. it can be constructed by one or many readable {@link ByteBuf}, reference count of these
 * buffers is not retained, and are managed by {@link DynamicCompositeByteBuf}. when dynamic
 * composite byte buffer release, it will release every sub buffers. when the readable size of a sub
 * buffer is zero, it will be removed from dynamic composite byte buffer, and will be release.
 * Currently, DynamicCompositeByteBuf does not support to write bytes, because netty {@link ByteBuf}
 * has satisfied already.
 */
public class DynamicCompositeByteBuf {
  ...
  /**
   * convert {@link DynamicCompositeByteBuf} to netty {@link ByteBuf}, the reference count of its
   * underlying buffers are not increased.
   *
  ...
  /**
   * add netty {@link ByteBuf} to {@link DynamicCompositeByteBuf}. the reference count of netty byte
   * buffer will be managed by {@link DynamicCompositeByteBuf}.
   *
  ...
  /**
   * Returns a new {@link CompositeByteBuf} which slice the first {@code length} of this composite
   * byte buffer and increases the {@code readerIndex} by the size of the new slice (= {@code
   * length}).
   *
  ...
  /**
   * Returns a new {@link CompositeByteBuf} which slice the first {@code length} of this composite
   * byte buffer while they maintain separate indexes and marks. This method does not modify {@code
   ...
   * {@code length} is greater than {@code this.readableBytes}, or if {@code dstIndex + length} is
