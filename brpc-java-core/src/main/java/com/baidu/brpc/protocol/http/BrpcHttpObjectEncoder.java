...
public abstract class BrpcHttpObjectEncoder<H extends HttpMessage> {
  ...
  private static final ByteBuf CRLF_BUF = unreleasableBuffer(
      directBuffer(2).writeByte(CR).writeByte(LF));
  private static final ByteBuf ZERO_CRLF_CRLF_BUF = unreleasableBuffer(
      directBuffer(ZERO_CRLF_CRLF.length)
  ...
  protected void encode(Object msg, List<Object> out) throws Exception {
    ...
    if (msg instanceof HttpMessage) {
      ...
      buf = new CompositeByteBuf(UnpooledByteBufAllocator.DEFAULT, false,
          (int) headersEncodedSizeAccumulator);
   ...
   */
  protected void sanitizeHeadersBeforeEncode(@SuppressWarnings("unused") H msg,
