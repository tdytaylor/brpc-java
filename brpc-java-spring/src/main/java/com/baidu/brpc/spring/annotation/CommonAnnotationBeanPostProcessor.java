...
/**
 * Common annotation bean post processor. it uses {@link AnnotationParserCallback}<br> interface to
 * define specified {@link Annotation} then recognize the Class to do <br> bean define action
 *
 * @author xiemalin
...
public class CommonAnnotationBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
   ...
   * <p>
   * Also allows for replacing the property values to apply, typically through creating a new
   ...
   * @param beanName the name of the bean
   * @return the actual property values to apply to to the given bean (can be the passed-in
   ...
   */
  protected void parseMethods(final Class<?> clazz,
      final List<Class<? extends Annotation>> annotions,
      ...
      public void doWith(Method method) {
        for (Class<? extends Annotation> anno : annotions) {
          ...
          if (annotation != null && method
              .equals(ClassUtils.getMostSpecificMethod(method, clazz))) {
   ...
   */
  protected void parseFields(final Class<?> clazz,
      final List<Class<? extends Annotation>> annotions,
      final LinkedList<InjectionMetadata.InjectedElement> elements) {
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    propertyResource = PropertyPlaceholderConfigurerTool
        .getRegisteredPropertyResourceConfigurer(beanFactory);
    if (resolver == null) {
      ...
      if (started.compareAndSet(false, true)) {
        for (BeanInfo bean : typeAnnotationedBeans) {
          if (getCallback() != null) {
            ...
            getCallback()
                .annotationAtTypeAfterStarted(bean.annotation, targetBean, bean.name, beanFactory);
          }
        }
      } else {
  ...
  private class AutowiredFieldElement extends InjectionMetadata.InjectedElement {
    ...
    protected void inject(Object bean, String beanName, PropertyValues pvs) throws Throwable {
      ...
      try {
        ...
        value = getCallback()
            .annotationAtField(annotation, value, beanName, pvs, beanFactory, field);
        if (value != null) {
          ReflectionUtils.makeAccessible(field);
    ...
    protected void inject(Object bean, String beanName, PropertyValues pvs) throws Throwable {
      ...
      try {
        ...
        for (int i = 0; i < arguments.length; i++) {
          ...
          arguments[i] =
              getCallback()
                  .annotationAtMethod(annotation, bean, beanName, pvs, beanFactory, method);
