...
/**
 * 字符串占位符解析工作类.<br> 例如: String testStr = "abc${ab}" 如果ab="ddd",则解析后结果为 abcddd<br>
 * <pre>
 * example:
...
public class PlaceholderResolver {
  ...
  /**
   * Parse the given String value recursively, to be able to resolve nested placeholders (when
   * resolved property values in turn contain placeholders again).
   *
   * @param strVal the String value to parse
   * @param visitedPlaceholders the placeholders that have already been visited during the current
   * resolution attempt (used to detect circular references between placeholders). Only non-null if
   * we're parsing a nested placeholder.
   * @return the string
   */
